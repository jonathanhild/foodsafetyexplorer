...
from .mappings import Inspection
...
class InspectionBuilder:
    ...
    def to_csv(self, filename=None):
        ...
        with open(filepath, 'w') as f:
            ...
            for row in tqdm(iterable=self.data,
                            desc='Writing inspection records to CSV file',
                            leave=True):
                w.writerow(list(row))
    ...
    def to_db(self, session):
        for d in self.data:
            session.add(d)
        print('Inserting inspection records into database.')
        session.commit()


class IInspectionBuilder:
    def __init__(self):
        self._instance = None

    def __call__(self, **kwargs):
        if not self._instance:
            self._instance = InspectionBuilder(**kwargs)
        return self._instance


factory.register_builder('INSPECTIONS', IInspectionBuilder())
